...
public class FilteringExample {
   ...
   public static void main(String[] args) {
      ...
      /*
       * [filter 메소드]
       *  - Stream<T> filter(함수형 인터페이스인 Predicate<? super T>를 매개변수로 갖음)
       *  - 새로운 스트림을 반환함.
       * [collect 메소드] 
      ...
      List<Member> developerList = list.stream()
         .filter(m -> m.getJob().equals("개발자"))   //잡이 개발자인 객체만 걸러서  새로운 스트림 생성
         .collect(Collectors.toList());   // 필터로 걸러진 스트림의 요소들을 List 컬렉션에 모아줌.
      ...
      // [1]과 [2]를 한번에 처리 // 메소드가 길게 연결된것을 메소드 체인 이라고 함
