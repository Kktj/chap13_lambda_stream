...
public class FilteringExample {
	...
	public static void main(String[] args) {
		...
		List<Member> list = Arrays.asList( // Arrays.asList 알아서 add 역할을 해줌
		...
		double ageAvg = list.stream()
				.filter((Member m) -> m.getSex() == Member.MALE) // 남성 객체로만 구성된 스트림 생성
				.mapToInt(Member :: getAge) // Member 객체를 나이로 매핑하는 IntStream 생성
				//.mapToInt(m -> m.getAge())
				.average() // 요소들의 평균 나이 계산해서 OptionalDouble 타입의 하나의 값으로 산출
