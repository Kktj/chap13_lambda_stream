...
* - 람다 함수는 익명 구현 객체이다. 객체이기 때문에 저장해서 사용할 변수가 필요함
* - 익명 구현 객체를 저장하기 위해서 인터페이스형 함수가 필요하다.
*/ 

public class Lamdba002 {

	public static void main(String[] args) {
		int max = 0;
		
		MyFunctionalInterface fi; // 함수형 인터페이스 타입 변수 , f1은 선언만 한 상태
		
		// 익명 구현 클래스 정의와 동시에 객체 생성
		fi = new MyFunctionalInterface() { // MyFunctionalInterface 이름만 사용
			@Override // 부모 인터페이스가 갖고 있는 max() 오버라이딩 = 부모와 똑같이 , 오버로딩 = 타!개!순
			public int max(int a, int b) {
				return a > b ? a : b;
			}
		};
		int result = fi.max(2 , 3); // f1에는 자식 객체 주소가 들어있음
		System.out.println("result : " + result);
		System.out.println(fi);
		System.out.println();
		/*
		 * [람다함수 - 익명 구현 객체]
		 * - 람다 함수를 정의하면 실행시 해당 코드가 실행됨과 동시에 바로 익명 객체로
		...
		// 람다식으로 선언하여 f1에 저장한 익명 구현 객체의 max()호출
		max = fi.max(2 , 3);
		System.out.println("람다식으로 선언한 익명 구현 객체(f1)의 max() 호출 : " + max);
		System.out.println(fi);
	}
